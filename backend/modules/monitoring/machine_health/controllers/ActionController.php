...
$where="tx.tgl1 > (DATE_SUB(CURDATE(), INTERVAL 3 YEAR)) or tx.tgl2 > (DATE_SUB(CURDATE(), INTERVAL 3 YEAR)) or tx.tgl3 > (DATE_SUB(CURDATE(), INTERVAL 3 YEAR)) or tx.tgl4 > (DATE_SUB(CURDATE(), INTERVAL 3 YEAR)) or tx.tgl5 > (DATE_SUB(CURDATE(), INTERVAL 3 YEAR))";

$datatables = (new DataTables\Builder())
    ->withQueryBuilder(
        $conn->createQueryBuilder()
            ->select("tx.*")
            ->from('tbl_unit_control','tx')
            ->where($where)
    )
